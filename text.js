...
let tier6 =
  "Antoni Gaudi, Saul Bass, James Dyson, Marcel Breuer, Charles and Ray Eames, Philippe Starck, Ludwig Mies van der Rohe, Aleksandr Rodchenko, Dieter Rams, Le Corbusier, Walter Gropius, Sir Jonathan Ive, Louis Kahn, David Adjaye, Thomas Heatherwick, Daniel Libeskind, Bjarke Ingels, Elizabeth Scofidio, Tadao Ando, Craig Edward Dykers and Kjetil Trædal Thorsen";

for (const x of tier1.split(", "))
  names.push({ name: x.replace(" ", "&nbsp;") + " ", tier: "1" });
for (const x of tier2a.split(", "))
  names.push({ name: x.replace(" ", "&nbsp;") + " ", tier: "2a" });
for (const x of tier2b.split(", "))
  names.push({ name: x.replace(" ", "&nbsp;") + " ", tier: "2b" });
for (const x of tier3.split(", "))
  names.push({ name: x.replace(" ", "&nbsp;") + " ", tier: "3" });
for (const x of tier4.split(", "))
  names.push({ name: x.replace(" ", "&nbsp;") + " ", tier: "4" });
for (const x of tier5.split(", "))
  names.push({ name: x.replace(" ", "&nbsp;") + " ", tier: "5" });
for (const x of tier6.split(", "))
  names.push({ name: x.replace(" ", "&nbsp;") + " ", tier: "6" });
